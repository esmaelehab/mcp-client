...
from typing import Optional, Any  # For type annotations, e.g., indicating that a variable may be None
...
class MCPClient:
    ...
    async def tool_arg(self) -> dict[str, Any]:
    ...
    async def process_query(self, query: str) -> str:
        ...
        """
        image_base64 = await self.tool_arg()
        image_url = "https://replicate.delivery/pbxt/HtKMvJSvuGWDn2B35mM396QGzcrgCNkcgSko8JxtXux4aX9H/sketch-mountains-input.jpeg"

        # Create a system prompt to guide the LLM's behavior
        system_prompt = f"""You are an AI assistant that can help with various tasks on image.
When a user asks to perform a task on an image you should:
1. Recognize that you have an image available in this url {image_url} and you can use it to perform the task.
2. Look at the available tools and choose the appropriate one for the task.
3. Return only a confirmation that the tool has been excuted.

For other tasks, use the appropriate tools as needed."""

        # Combine system prompt and user query
        combined_prompt = f"{system_prompt}\n\nUser query: {query}"

        # Create Gemini Content object with the combined prompt
        ...
        # Process each candidate response from Gemini.
            ...
            if candidate.content.parts:  # Ensure that the response has parts (sections).
                for part in candidate.content.parts:
                    ...
                    if part.function_call:
                        ...
                        # If the tool requires an image, add the image URI
                        #if "image_base64" in tool_args:
                        #    tool_args["image_base64"] = image_base64
                        #    print("Added image base64 to tool args")
                        ...
                        try:
                            ...
                            print(f"Tool response: {result.content}")  # Debug print
                        ...
                        print("Sending response back to Gemini...")
                        # Send the original query, the function call, and the tool response back to Gemini.
                        response = self.genai_client.models.generate_content(
                            model='gemini-2.0-flash-001',
                            contents=[
                                user_prompt_content,       # Original combined prompt
                        ...
                        print("Received final response from Gemini")

                        # Append the text from the first part of Gemini's new candidate response.
                        if response.candidates and response.candidates[0].content.parts:
                            response_text = response.candidates[0].content.parts[0].text
                            print(f"Final response text: {response_text}")  # Debug print
                            final_text.append(response_text)
                        else:
                            print("Warning: No response text found in Gemini's response")
                    else:
                        # If there is no function call, just use the text provided by Gemini.
                        print("No function call, using direct text")
                        final_text.append(part.text)

        # Combine all parts of the response into a single string to be returned.
        return "\n".join(final_text)
